# 6.1 Boundary Foundations

_Page Type: Chapter Index | Maturity: Draft_

This section establishes the distinctions that keep privacy, sovereignty, and portability questions from being flattened into one vague "data concern."

## Section Map

- 6.1.1 [Data Classes, Residency, And Control Distinctions](06-01-01-data-classes-residency-and-control-distinctions.md)
- 6.1.2 [Decision Boundaries And Handling Heuristics](06-01-02-decision-boundaries-and-handling-heuristics.md)

## What This Section Helps Clarify

- what counts as part of the real data boundary
- where privacy obligations stop being enough to answer sovereignty questions
- when export, deletion, support access, or telemetry posture should change the architecture decision

Back to [6. Data Sovereignty And Privacy](06-00-00-data-sovereignty-and-privacy.md).
