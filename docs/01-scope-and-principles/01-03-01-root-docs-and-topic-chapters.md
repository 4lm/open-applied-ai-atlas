...
The key boundary is simple: root guidance explains the durable repository contract; optional prompt artifacts in `pips/` hold task-specific planning context when a task explicitly uses them; numbered chapter files explain applied organizational AI and ML itself. The hard part is spotting mixed material early enough that mission, editorial rules, and temporary prompt context do not get copied into the wrong layer.
...
| --- | --- | --- | --- |
| Root guidance | "How does this repository work, and what durable contract should contributors follow?" | mission-led navigation, contribution workflow, editorial rules, changelog policy, durable agent instructions | chapter-deep explainers, product-style long topical surveys, or controller-specific runtime state |
