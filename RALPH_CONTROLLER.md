...
- Ralph checks observed live research activity against claimed planning research and execution verification before accepting those blocks as complete, but it does not use a fixed observed-search target. That validation is phase-scoped rather than main-turn-only: live research performed in the main turn or in auxiliary `research` workers counts toward the same planning or execution attempt. When telemetry is missing, Ralph can still accept a completed block if the returned findings and sources provide coherent external grounding strong enough for the declared `live_search_assessment.depth`.
