...
`./scripts/ralph-codex.py` is the repository's human-run outer controller for long-running Codex work. It launches `codex app-server`, stores structured session state under `.codex/ralph-codex/`, starts in Plan mode to build a broadened execution charter, and resumes prior work only through explicit session commands.
