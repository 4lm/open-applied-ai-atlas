...
class RepoContractTests(unittest.TestCase):
    ...
    def test_ralph_controller_documents_canonical_contract(self):
        ...
        self.assertIn("quality-first prompt shaping", ralph_text)
        self.assertIn("best viable prompt", ralph_text)
        self.assertIn("same_tranche_escalation_after", ralph_text)
        self.assertIn("acceptance_confidence_floor", ralph_text)
        self.assertIn("only profile-visible hard whole-run fuse", ralph_text)
        ...
        self.assertIn("progress_checkpoint_mode", ralph_text)
        self.assertIn("milestone_policy", ralph_text)
        self.assertIn("worker_policy", ralph_text)
        self.assertIn("turn_timeout_secs", ralph_text)
        self.assertIn("worker-summary-output-v0.1.0.schema.json", ralph_text)
        self.assertNotIn("disjoint_execution", ralph_text)
        self.assertIn("evidence_policy", ralph_text)
        self.assertIn("evidence_assessment", ralph_text)
        self.assertIn("live_search_assessment", ralph_text)
        ...
        self.assertNotIn("max_planning_repairs", ralph_text)
        self.assertNotIn("raw_task_revisit_threshold", ralph_text)
